...
import { TOOL_NAME, ToolInputParams, ToolReturnParams } from './models/types.js'
import { getEnv } from './service/env.js'
import { getServerDescription, listResources, readCollectionsInfo } from './service/resources.js'
import * as tools from './service/tools/index.js'
...
server.setRequestHandler(mcpTypes.ReadResourceRequestSchema, async (request): Promise<mcpTypes.ReadResourceResult> => {
...
server.setRequestHandler(mcpTypes.ListToolsRequestSchema, async () => {
  return {
    tools: [
      tools.addCollectionTypeSchema,
      tools.addToCollectionSchema,
      tools.getFromCollectionSchema,
      tools.deleteFromCollectionSchema,
      tools.getCollectionSummarySchema,
      tools.getResourceDataSchema
    ]
  }
})

server.setRequestHandler(mcpTypes.CallToolRequestSchema, async (request): Promise<mcpTypes.CallToolResult> => {
  ...
  switch (request.params.name) {
    case TOOL_NAME.ADD_COLLECTION_TYPE:
      handler = tools.addCollectionType
      break

    case TOOL_NAME.ADD_TO_COLLECTION:
      handler = tools.addToCollection
      break

    case TOOL_NAME.GET_FROM_COLLECTION:
      handler = tools.getFromCollection
      break

    case TOOL_NAME.DELETE_FROM_COLLECTION:
      handler = tools.deleteFromCollection
      break

    case TOOL_NAME.COLLECTION_SUMMARY:
      handler = tools.getCollectionSummary
      break

    case TOOL_NAME.GET_RESOURCE_DATA:
      handler = tools.getResourceData
      break
